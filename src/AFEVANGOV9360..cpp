...
// Constructor
AFEVANGOV9360::AFEVANGOV9360(HardwareSerial& port, uint8_t receivePin, uint8_t transmitPin)
{
    port.begin(AFE_BAUD_RATE, SERIAL_8O1, receivePin, transmitPin);
...
uint32_t AFEVANGOV9360::readdata(AFECommand cmd, AFERegister reg) {
    ...
    Serial2.write(AFE_HEADER);
    Serial2.write(cmd);
    Serial2.write(cmd1);
    Serial2.write(checksum);
    Serial2.flush();  // Ensure all bytes are transmitted

    unsigned long startTime = millis();
    while (Serial2.available() < 5 && (millis() - startTime) < 100) {
        delay(1);
    }
    ...
    uint8_t received_cksum = Serial2.read();
...
bool AFEVANGOV9360::writedata(uint32_t data, AFERegister reg) {
    ...
    Serial2.write(AFE_HEADER);
    Serial2.write(AFE_CMD_01);
    Serial2.write(cmd4);
    Serial2.write(data_bytes, 4);
    Serial2.write(checksum);
    Serial2.flush();

    unsigned long startTime = millis();
    while (Serial2.available() < 1 && (millis() - startTime) < 100) {
        delay(1);
    }
